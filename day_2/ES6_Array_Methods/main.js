...
// console.log(vegetables.filter(v => (v.color === "orange")))
...
// const increaseSalary = p => {if(p.goodPerformance){p.salary += 300}}
...
// console.log(messagesFromDad.map(str => str.toLowerCase()))
...
// movies.every(m => m.syear > 2020) ? console.log("Futuristic stuff"):console.log("Yawn");
...
//Exercise 1
// let newArray = users.map(user => {  return {emapil: user.email,companyName: user.companyName} })
// console.log(newArray)

//Exercise 2 + 3
// let startWith5 = users.filter(u => (u.address.zipcode[0] === "5")).map(u => u.id);
// console.log(startWith5)

//Exercise 4
// let names = users.map(u => u.name);
// let newNames = names.filter( name => ( name.startsWith("C") ) );
// console.log(names)
...
// Exercise 6
// let employee = users.find(u => u.address.suite ===  "Apt. 950"); 
// console.log(employee.company.name)

// Exercise 7
const myFunc = user =>{
    console.log(`${user.name} lives in ${user.address.city} and owns the ${user.company.name}`)
}
